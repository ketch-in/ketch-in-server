...
var RTCMultiConnection = function (roomid, forceOptions) {
  ...
  function SocketConnection(connection, connectCallback) {
    ...
    connection.socket.on("extra-data-updated", function (remoteUserId, extra) {
      console.log("> extra-data-updated", remoteUserId, extra);

      console.log(
        "> extra-data-updated STEP 1",
        connection.peers[remoteUserId]
      );
      console.log(connection.peers.getAllParticipants());
      if (!connection.peers[remoteUserId]) return;

      console.log("> extra-data-updated STEP 2");
      ...
      console.log("> extra-data-updated STEP 3");

      updateExtraBackup(remoteUserId, extra);
    });

    function updateExtraBackup(remoteUserId, extra) {
      console.log("> extra-data-updated STEP 4", remoteUserId, extra);
      if (!connection.peersBackup[remoteUserId]) {
        console.log("> extra-data-updated STEP 5");
        connection.peersBackup[remoteUserId] = {
          userid: remoteUserId,
          extra: {},
        };
      }
      console.log("> extra-data-updated STEP 6");
      connection.peersBackup[remoteUserId].extra = extra;
    }

    function onMessageEvent(message) {
      console.log("> file-sharing-demo", message);
    ...
    connection.socket.on("closed-entire-session", function (sessionid, extra) {
      console.log("> closed-entire-session", sessionid, extra);
      connection.leave();
    ...
    connection.socket.on(
      ...
      function (data) {
        console.log("> number-of-broadcast-viewers-updated", data);
        connection.onNumberOfBroadcastViewersUpdated(data);
      }
    );

    connection.socket.on("set-isInitiator-true", function (sessionid) {
      console.log("> set-isInitiator-true", sessionid);
  ...
  (function (connection) {
    ...
    connection.openOrJoin = function (roomid, callback) {
      ...
      connection.checkPresence(roomid, function (isRoomExist, roomid) {
        console.log("checkPresence - step3", isRoomExist, roomid);
        if (isRoomExist) {
    ...
    connection.open = function (roomid, callback) {
      console.log("> open", roomid, callback);
      callback = callback || function () {};
    ...
    function openRoom(callback) {
      ...
      connection.socket.emit(
        ...
        function (isRoomOpened, error) {
          console.log("> open-room STEP1", isRoomOpened, error);
    ...
    connection.checkPresence = function (roomid, callback) {
      console.log("> checkPresence", roomid, SocketConnection.name);
      ...
      if (!connection.socket) {
        connection.connectSocket(function () {
          console.log("> connectSocket7");
          connection.checkPresence(roomid, callback);
        });
        return;
      }
      console.log("> checkPresence - step1");
      connection.socket.emit(
        "check-presence",
        roomid + "",
        function (isRoomExist, _roomid, extra) {
          console.log("> checkPresence - step2", isRoomExist, _roomid, extra);
          if (connection.enableLogs) {
            console.log(
